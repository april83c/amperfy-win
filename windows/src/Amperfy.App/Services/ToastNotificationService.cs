...
public static class ToastNotificationService
{
    ...
    private static void NotificationInvoked(AppNotificationManager sender, AppNotificationActivatedEventArgs args)
    {
        ...
        _dispatcher?.TryEnqueue(() => HandleActivation(arguments));
    }

    /// Handles the arguments of a clicked notification (also used when the click launched the app). UI thread.
    public static void HandleActivation(IReadOnlyDictionary<string, string> arguments)
    {
        if (_services is null) return;
        try
        {
            _services.MainWindow?.BringToFront();
            if (!arguments.TryGetValue(ArgAccount, out var accountIdent) ||
                !arguments.TryGetValue(ArgEpisode, out var episodeId)) return;
