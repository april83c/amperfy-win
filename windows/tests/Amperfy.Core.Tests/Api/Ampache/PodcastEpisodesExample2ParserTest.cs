...
public class PodcastEpisodesExample2ParserTest : AbstractAmpacheTest
{
    private readonly Podcast _testPodcast;

    public PodcastEpisodesExample2ParserTest()
    {
        XmlData = GetTestFileData("podcast_episodes_example_2");
    ...
    protected override void CheckCorrectParsing()
    {
        ...
        Assert.Equal(1638223676, new DateTimeOffset(episode.PublishDate).ToUnixTimeSeconds()); // "2021-11-29T22:07:56+00:00"
        ...
        Assert.Equal(1637014020, new DateTimeOffset(episode.PublishDate).ToUnixTimeSeconds()); // "2021-11-15T22:07:00+00:00"
        Assert.Null(episode.StreamId);
        ...
        Assert.Equal(1636409977, new DateTimeOffset(episode.PublishDate).ToUnixTimeSeconds()); // "2021-11-08T22:19:37+00:00"
        Assert.Null(episode.StreamId);
        Assert.Equal(PodcastEpisodeRemoteStatus.Downloading, episode.PodcastStatus);
        AssertTestAccount1(episode.Podcast?.Account);
        Assert.Same(podcast, episode.Podcast);
