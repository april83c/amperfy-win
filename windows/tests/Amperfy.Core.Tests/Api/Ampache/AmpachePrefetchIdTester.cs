...
/// Port of PrefetchIdTester.swift (own copy for the Ampache tests).
public sealed class AmpachePrefetchIdTester
{
    private readonly LibraryStorage _library;
    private readonly PrefetchIdContainer _prefetchIDs;

    public AmpachePrefetchIdTester(LibraryStorage library, PrefetchIdContainer prefetchIDs)
    {
        _library = library;
    ...
    {
        var summedCount = artworkCount + genreIdCount + genreNameCount + artistCount + localArtistCount + albumCount +
                          songCount + podcastEpisodeCount + radioCount + musicFolderCount + directoryCount + podcastCount;
        ...
        var summedFetchCount = actArtworkFetchCount + actGenreFetchCount + actArtistFetchCount + actLocalArtistFetchCount +
